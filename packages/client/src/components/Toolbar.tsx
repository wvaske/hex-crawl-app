  ...
  return (
    <div className="bg-ink-900/95 border border-ink-700 rounded-lg p-2 shadow-xl backdrop-blur w-56 text-xs text-ink-300 space-y-2 overflow-y-auto">
      ...
      <label className="block">
        <span className="block text-[0.625rem] text-ink-400">Bar distance (miles)</span>
        <Input
          type="number"
          min={0}
          step="any"
      ...
      <label className="block">
        <span className="block text-[0.625rem] text-ink-400">Miles per hex</span>
        <Select
...
function StickerPicker() {
  ...
  return (
    <div className="bg-ink-900/95 border border-ink-700 rounded-lg p-2 shadow-xl backdrop-blur w-60 overflow-y-auto">
      ...
      />
      {groups.length === 0 && (
        <p className="text-[0.6875rem] text-ink-400 mb-2">No sticker matches.</p>
      )}
      {groups.map((group) => (
