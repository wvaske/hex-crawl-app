...
describe('integration content upsert delivers discoveries', () => {
  it('a passive clue opened by an AI-added location reaches the journal, not just the table', async () => {
      ...
      clues: [
        {
          ...
          gate: { kind: 'skill', skill: 'perception', dc: 10, maxDistance: 4, mode: 'passive' },
    ...
    const clueId = [...runtime.requireMap(mapId).contents.values()].find(
      (c) => c.title === 'Adderstand',
    )!.clues[0]!.id;
    ...
    const dmLine = runtime.log.find(
      (e) => e.kind === 'discovery' && e.visibility === 'dm' && e.data.clueId === clueId,
    );
    ...
    const playerLine = runtime.log.find(
      (e) => e.kind === 'discovery' && ownerSeats.includes(e.visibility),
    );
