...
2. Mount invitation routes in `packages/server/src/app.ts`:
   - The invitation routes contain both `/campaigns/:campaignId/invitations` and `/invitations/*` paths
   - Use `app.route("/api", invitations)` so the full paths are `/api/campaigns/:campaignId/invitations` and `/api/invitations/*`
   - Alternatively, mount at two different base paths. Choose the approach that keeps routes clean.
   - The simplest approach: create separate sub-routers or use a single router mounted at `/api` with full paths defined inside.
  </action>
  <verify>
