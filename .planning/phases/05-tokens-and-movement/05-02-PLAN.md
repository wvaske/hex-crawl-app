...
3. Adjacency: if role !== "dm", parse both hexKeys using parseHexKey from shared, compute cube distance inline (convert offset to cube coords, then `(abs(dq)+abs(dr)+abs(ds))/2`). hexDistance currently only exists in packages/client/src/hex/neighbors.ts and cannot be imported server-side, so implement the distance check directly: convert the two hex offset coords to cube coords and check distance === 1. Reject with error "Can only move to adjacent hexes" if distance !== 1.
4. Update token hexKey in DB
5. Broadcast token:moved to all clients via broadcastToAll
...
Import parseHexKey from shared. Implement cube-distance calculation inline (do NOT import from client package — server cannot access client code). Use the same db import pattern as fog-utils.ts.
  </action>
